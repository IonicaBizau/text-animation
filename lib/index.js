...
var Figlet = require("figlet")
  ...
  , Term = require('terminal-control').init().autoClean();
  ;
...
var TextAnimation = module.exports = function (options, callback) {
    ...
    if (/^[0-9]+x[0-9]+$/.test(options.size)) {
        ...
        options.size = [process.stdout.columns, process.stdout.rows];
    }
    ...
    Figlet.text(text, {
        ...
        try {
            renderer = require("./animations/" + options.animation)(options);
        } catch (e) {
        ...
        var animate = function animate(frame) {
            var display = renderer(frame);
            if (!display) {
                options.end();
            } else {
                process.stdout.write("\n");
                Term.moveCursorUp(options.size[1]);
